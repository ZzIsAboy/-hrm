...
<body>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td class="main_locbg font2"><img src="${pagePath}/page/static/images/pointer.gif">&nbsp;当前位置：用户管理&nbsp;&gt;&nbsp;用户查询</td>
					...
					<tr>
						<td class="fftd">
							<form name="empform" method="post" id="empform" action="${pagePath}/user/list.do">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td class="font3">用户名：<input type="text" name="key" value="${key}"> 用户状态：<select name="status" style="width: 173px;">
				...
				<table width="100%" border="1" cellpadding="5" cellspacing="0" style="border: #c2c6cc 1px solid; border-collapse: collapse;">
					...
					<c:forEach items="${pages.list}" var="user" varStatus="x">
						<tr id="data_${x.index}" align="center" class="main_trbg" onMouseOver="move(this);" onMouseOut="out(this);">
							...
							<td>${user.password}</td>
							<td>${user.realname}</td>
							<td><c:if test="${user.status == 1}">已审核</c:if> <c:if test="${user.status == 2}">未审核</c:if></td>
							<td><fmt:formatDate value="${user.createTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td align="center" width="40px;"><a href="${pagePath}/user/modify.do?flag=1&id=${user.id}"> <img title="修改" src="${pagePath}/page/static/images/update.gif" /></a></td>
						</tr>
					</c:forEach>
