			...
			if (checkedBoxs.length < 1) {
				...
				$.ligerDialog.confirm("确认要删除吗?", "删除部门", function(r) {
					if (r) {
						// alert("删除："+ids.get());
						// 发送请求
						window.location = "${pagePath}/department/delete.do?ids=" + ids.get();
					}
				});
			}
		});
...
<body>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td class="main_locbg font2"><img src="${pagePath}/page/static/images/pointer.gif">&nbsp;当前位置：部门管理&nbsp;&gt;&nbsp;部门查询</td>
		...
		<tr valign="top">
			<td height="30">
				<table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
					<tr>
						<td class="fftd">
							<form name="deptform" method="post" id="deptform" action="${pagePath}/department/list.do">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td class="font3">部门名称：<input type="text" name="name" value="${name}"> <input type="submit" value="搜索" /> <input type="button" onclick="location.href='${pagePath}/department/list.do'" value="清空" /> <input id="delete" type="button" value="删除" />
										</td>
									</tr>
								</table>
							</form>
		...
		<tr valign="top">
			<td height="20">
				<table width="100%" border="1" cellpadding="5" cellspacing="0" style="border: #c2c6cc 1px solid; border-collapse: collapse;">
					...
					<c:forEach items="${pages.list}" var="department" varStatus="x">
						<tr id="data_${x.index}" align="center" class="main_trbg" onMouseOver="move(this);" onMouseOut="out(this);">
							<td><input type="checkbox" id="box_${x.index}" value="${department.id}"></td>
							<td>${department.name}</td>
							<td>${department.remark}</td>
							<td align="center" width="40px;"><a href="${pagePath}/department/modify.do?flag=1&id=${department.id}"> <img title="修改" src="${pagePath}/page/static/images/update.gif" /></a></td>
						</tr>
					</c:forEach>
					...
					<table width="100%" align="center" style="font-size: 13px;" class="sabrosus">
						<tr>
							<td style="color: #0061de; margin-right: 3px; padding-top: 2px; text-decoration: none"><c:if test="${pages.hasPreviousPage}">
									<a href="${pagePath}/department/list.do?page=${pages.pageNum - 1}&name=${name}"> 上一页</a>
									...
									<c:if test="${pageNum != pages.pageNum}">
										<a href="${pagePath}/department/list.do?page=${pageNum}&name=${name}">${pageNum}</a>
									</c:if>
								</c:forEach> <c:if test="${pages.hasNextPage}">
									<a href="${pagePath}/department/list.do?page=${pages.pageNum + 1}&name=${name}"> 下一页</a>
	...
	<script type="text/javascript">
		document.getElementById("pager_jump_btn").onclick = function(){
			...
			if (!/^[1-9]\d*$/.test(page_size)||page_size<1||page_size>${pages.pages}){
				...
				var submit_url = "${pagePath}/department/list.do?page=" + page_size + "&name=${name}";
