	...
	$(function() {
			...
			if (checkedBoxs.length < 1) {
				...
				$.ligerDialog.confirm("确认要删除吗?", "删除公告", function(r) {
					if (r) {
						// alert("删除："+ids.get());
						// 发送请求
						window.location = "${pagePath}/notice/delete.do?ids=" + ids.get();
					}
				});
			}
		})
		...
		$("a[id^='prev_'").click(function() {
			...
			$.ligerDialog.open({
				title : '预览公告',
				height : 500,
				url : '${pagePath}/notice/preview.do?id=' + noticeId,
				width : 750,
				showMax : true,
				showToggle : true,
				showMin : true,
...
<body>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td class="main_locbg font2"><img src="${pagePath}/page/static/images/pointer.gif">&nbsp;当前位置：公告管理&nbsp;&gt;&nbsp;公告查询</td>
		...
		<tr valign="top">
			<td height="30">
				<table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
					<tr>
						<td class="fftd">
							<form name="noticeform" method="post" id="noticeform" action="${pagePath}/notice/list.do">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td class="font3">公告标题：<input type="text" name="title" value="${title}"> 公告内容：<input type="text" name="content" value="${content}"> <input type="submit" value="搜索" /> <input type="button" onclick="location.href='${pagePath}/notice/list.do'" value="清空" /> <input id="delete" type="button" value="删除" />
										</td>
									</tr>
								</table>
							</form>
		...
		<tr valign="top">
			<td height="20">
				<table width="100%" border="1" cellpadding="5" cellspacing="0" style="border: #c2c6cc 1px solid; border-collapse: collapse;">
					...
					<c:forEach items="${notices}" var="entity" varStatus="x">
						<tr id="data_${x.index}" align="center" class="main_trbg" onMouseOver="move(this);" onMouseOut="out(this);">
							<td><input type="checkbox" id="box_${x.index}" value="${entity.notice.id}"></td>
							<td>${entity.notice.title}</td>
							<td>${entity.notice.content}</td>
							<td><fmt:formatDate value="${entity.notice.createTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td>${entity.user.realname}</td>
							<td align="center" width="40px;"><a href="${pagePath}/notice/modify.do?flag=1&id=${entity.notice.id}"> <img title="修改" src="${pagePath}/page/static/images/update.gif" /></a></td>
							<td align="center" width="40px;"><a href="javascript:;" id="prev_${entity.notice.id}"> <img title="预览" src="${pagePath}/page/static/images/prev.gif" /></a></td>
						</tr>
					</c:forEach>
					...
					<table width="100%" align="center" style="font-size: 13px;" class="sabrosus">
						<tr>
							<td style="color: #0061de; margin-right: 3px; padding-top: 2px; text-decoration: none"><c:if test="${pages.hasPreviousPage}">
									<a href="${pagePath}/notice/list.do?page=${pages.pageNum - 1}&key=${key}&status=${status}"> 上一页</a>
									...
									<c:if test="${pageNum != pages.pageNum}">
										<a href="${pagePath}/notice/list.do?page=${pageNum}&key=${key}&status=${status}">${pageNum}</a>
									</c:if>
								</c:forEach> <c:if test="${pages.hasNextPage}">
									<a href="${pagePath}/notice/list.do?page=${pages.pageNum + 1}&key=${key}&status=${status}"> 下一页</a>
	...
	<script type="text/javascript">
		document.getElementById("pager_jump_btn").onclick = function(){
			...
			if (!/^[1-9]\d*$/.test(page_size)||page_size<1||page_size>${pages.pages}){
				...
				var submit_url = "${pagePath}/notice/list.do?page=" + page_size + "&key=${key}&status=${status}";
